...
system("docker stop mongobank redisbank postgresbank mssqlbank")
```
...
As with any `{cachem}` compatible objects, the cache can be manually flushed using the `$reset()` method -- this will call `drop()` on MongoDB, `FLUSHALL` in Redis, & `DBI::dbRemoveTable()` + `DBI::dbCreateTable()` with Postgre and Microsoft SQL Server. 
...
Microsoft SQL Server `varchar(max)` column can only store up to 2GB elements, so you can't write a cache that's > 2GB. 
...
```{r echo = FALSE, eval = TRUE, results = 'hide'}
system('docker run --rm --name mssqlbank -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=MySecret@Passw0rd" -p 1433:1433 -d mcr.microsoft.com/mssql/server:2019-latest')
Sys.sleep(20)
```
